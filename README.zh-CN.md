...
- 宿主 peer 依赖声明为 `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1 || 0.1.5-rc.2 || 0.1.7-rc.1 || 0.1.7-rc.2`。
- Node.js >= 22.19，PATH 中可执行 Git。
- 当前会话关联本地 Git 项目。

## 安装

当前版本为 `0.1.10`。以下命令使用 `web` profile，请按实际环境替换。安装前停用其他注册 `/simplify` 的插件。
...
```text
请将 DSH 插件 @michengai/dsh-simplify 安装到本机 web profile，执行：dsh plugin --profile web add @michengai/dsh-simplify@0.1.10 --registry=https://registry.npmjs.org/。安装后执行 dsh --profile web --dump-config，确认配置包含 michengai-simplify，并告诉我如何重新加载 DSH 和开始使用。
```
...
从 [GitHub Release](https://github.com/MichengAI/dsh-simplify/releases/tag/v0.1.10) 下载已发布的 tgz，然后在安装包所在目录执行：
